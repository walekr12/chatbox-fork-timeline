...
import { ActionIcon, Badge, Button, Divider, Flex, Modal, ScrollArea, Stack, Text, Transition } from '@mantine/core'
...
const MessageList = forwardRef<MessageListRef, MessageListProps>((props, ref) => {
  ...
  const [forkTimelineOpened, setForkTimelineOpened] = useState(false)
  ...
  const handleScrollToMessage = useCallback(
    (messageId: string) => {
      const index = renderItems.findIndex((item) => item.messages.some((message) => message.id === messageId))
      if (index >= 0) {
          ...
          {currentSession.messageForksHash?.[msg.id] && currentSession.messageForksHash[msg.id].lists.length > 1 && (
            <Flex justify="flex-end" pr="md" mr="md" className="self-end">
              <ForkNav
                ...
                onOpenTimeline={() => setForkTimelineOpened(true)}
              />
            </Flex>
          )}
        </Stack>
    ...
    <div className={cn('w-full h-full mx-auto', props.className)}>
      <BlockCodeCollapsedStateProvider defaultCollapsed={!!settingsStore.getState().autoCollapseCodeBlock}>
        <div className="overflow-hidden h-full pr-0 pl-1 sm:pl-0 relative" ref={messageListRef}>
          ...
          <ForkTimelineModal
            opened={forkTimelineOpened}
            session={currentSession}
            currentMessageList={currentMessageList}
            onClose={() => setForkTimelineOpened(false)}
            onJump={(messageId) => {
              setForkTimelineOpened(false)
              window.setTimeout(() => handleScrollToMessage(messageId), 100)
            }}
          />
...
function ForkNav(props: {
  ...
  const { sessionId, msgId, forks, onOpenTimeline } = props
...
type ForkTimelineModalProps = {
  opened: boolean
  session: Session
  currentMessageList: SessionMessage[]
  onClose: () => void
  onJump: (messageId: string) => void
}

function ForkTimelineModal(props: ForkTimelineModalProps) {
  const { opened, session, currentMessageList, onClose, onJump } = props
  const { t } = useTranslation()
  const rowsByFork = useMemo(() => buildForkTimelineRows(session, currentMessageList), [session, currentMessageList])
  const forkGroups = Object.entries(rowsByFork)

  const handleJump = useCallback(
    async (row: ForkTimelineRow) => {
      await switchForkToList(session.id, row.forkMessageId, row.branchIndex)
      onJump(row.forkMessageId)
    },
    [onJump, session.id]
  )

  const handleJumpLatest = useCallback(
    async (rows: ForkTimelineRow[]) => {
      const latest = rows.reduce((candidate, row) => (row.updatedAt > candidate.updatedAt ? row : candidate), rows[0])
      if (latest) {
        await handleJump(latest)
      }
    },
    [handleJump]
  )
  ...
  return (
    <Modal opened={opened} onClose={onClose} size="xl" title={t('Fork Timeline')} centered>
      <Stack gap="sm">
        <Text size="sm" c="chatbox-tertiary">
          {t('Double-click a fork node to switch to that branch and scroll to the fork point.')}
        </Text>
        <ScrollArea h={520} offsetScrollbars>
          <Stack gap="xs" pr="sm">
            {forkGroups.length === 0 ? (
              <Text c="chatbox-tertiary" ta="center" py="xl">
                {t('No forks yet')}
              </Text>
            ) : (
              forkGroups.map(([forkMessageId, rows]) => (
                <Stack key={forkMessageId} gap={6} className="border-l border-chatbox-border-primary pl-3">
                  <Flex
                    align="center"
                    justify="space-between"
                    gap="sm"
                    className="cursor-pointer rounded-md px-2 py-1 hover:bg-chatbox-brand/10"
                    onDoubleClick={() => void handleJumpLatest(rows)}
                  >
                    <Stack gap={2} className="min-w-0">
                      <Text size="sm" fw={600} lineClamp={1}>
                        {rows[0]?.forkMessagePreview || t('Fork point')}
                      </Text>
                      <Text size="xs" c="chatbox-tertiary">
                        {t('Branches')}: {rows.length}
                      </Text>
                    </Stack>
                    <Badge variant="light" color="chatbox-secondary">
                      {t('latest by time')}
                    </Badge>
                  </Flex>
                  <Stack gap={6} pl="md">
                    {rows.map((row) => (
                      <Flex
                        ...
                        gap="sm"
                        align="flex-start"
                        className="cursor-pointer rounded-md border border-chatbox-border-primary px-3 py-2 hover:bg-chatbox-brand/10"
                        onDoubleClick={() => void handleJump(row)}
                      >
                        <Text size="xs" c="chatbox-tertiary" className="w-14 shrink-0 pt-1">
                          #{row.branchIndex + 1}
                        </Text>
                        <Stack gap={3} className="min-w-0 flex-1">
                          <Flex gap="xs" align="center" wrap="wrap">
                            {row.isCurrent && (
                              <Badge size="xs" variant="filled" color="chatbox-secondary">
                                {t('current')}
                              </Badge>
                            )}
                            <Text size="xs" c="chatbox-tertiary">
                              {new Date(row.updatedAt).toLocaleString()} · {row.messageCount} {t('messages')}
                            </Text>
                          </Flex>
                          <Text size="sm" lineClamp={3} className="whitespace-pre-wrap">
                            {row.preview || t('Empty branch')}
                          </Text>
                        </Stack>
                      </Flex>
                    ))}
...
function buildForkTimelineRows(
  session: Session,
  currentMessageList: SessionMessage[]
): Record<string, ForkTimelineRow[]> {
  const result: Record<string, ForkTimelineRow[]> = {}
  if (!session.messageForksHash) {
    return result
  }

  const messageIndex = new Map(currentMessageList.map((message, index) => [message.id, index]))
  const sortedForkIds = Object.keys(session.messageForksHash).sort(
    (a, b) => (messageIndex.get(a) ?? Number.MAX_SAFE_INTEGER) - (messageIndex.get(b) ?? Number.MAX_SAFE_INTEGER)
  )

  for (const forkMessageId of sortedForkIds) {
    const forkEntry = session.messageForksHash[forkMessageId]
    const forkPoint = currentMessageList.find((message) => message.id === forkMessageId)
    const currentTail = getCurrentForkTail(currentMessageList, forkMessageId)
    result[forkMessageId] = forkEntry.lists.map((list, branchIndex) => {
      const messages = branchIndex === forkEntry.position ? currentTail : list.messages
      return {
        forkMessageId,
        forkMessagePreview: previewText(forkPoint ? getMessageText(forkPoint) : forkMessageId),
        branchIndex,
        listId: list.id,
        isCurrent: branchIndex === forkEntry.position,
        messageCount: messages.length,
        preview: previewText(messages.map((message) => getMessageText(message)).join('\n')),
        updatedAt: getForkListTime(list, messages, forkEntry.createdAt),
      }
    })
  }
...
function getCurrentForkTail(messages: SessionMessage[], forkMessageId: string) {
  const forkIndex = messages.findIndex((message) => message.id === forkMessageId)
  return forkIndex >= 0 ? messages.slice(forkIndex + 1) : []
}

function getForkListTime(
  list: NonNullable<Session['messageForksHash']>[string]['lists'][number],
  messages: SessionMessage[],
  fallback: number
) {
  return Math.max(
    ...
    list.createdAt ?? 0,
    fallback,
    ...messages.map((message) => message.updatedAt ?? message.timestamp ?? 0)
  )
}

function previewText(text: string, maxLength = 300) {
