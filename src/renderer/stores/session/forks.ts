...
/**
 * Switch directly to a specific fork branch.
 */
export async function switchForkToList(sessionId: string, forkMessageId: string, listIndex: number) {
  await chatStore.updateSessionWithMessages(sessionId, (session) => {
    if (!session) {
      throw new Error('Session not found')
    }
    const patch = buildSwitchForkToListPatch(session, forkMessageId, listIndex)
    if (!patch) {
      return session
    }
    return {
...
function buildSwitchForkToListPatch(
  session: Session,
  ...
  if (!forkEntry || listIndex < 0 || listIndex >= forkEntry.lists.length) {
    return null
  }

  const rootResult = switchForkInMessages(session.messages, forkEntry, forkMessageId, listIndex)
  if (rootResult) {
  ...
  const updatedThreads = session.threads.map((thread) => {
    ...
    const result = switchForkInMessages(thread.messages, forkEntry, forkMessageId, listIndex)
    if (!result) {
      return thread
    }
...
function switchForkInMessages(
  ...
  forkMessageId: string,
  directionOrIndex: 'next' | 'prev' | number
  ...
  const newPosition =
    typeof directionOrIndex === 'number'
      ? Math.min(Math.max(directionOrIndex, 0), total - 1)
      : directionOrIndex === 'next'
        ? (adjustedCurrentPosition + 1) % total
        : (adjustedCurrentPosition - 1 + total) % total

  if (!isCurrentBranchEmpty && newPosition === adjustedCurrentPosition) {
    return {
      messages,
      fork: {
        ...forkEntry,
